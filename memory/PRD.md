...
### Phase 14 - Vibe Chat → Real Agent Execution (nidoai Architecture on Supabase) (Complete - May 25, 2026)
1. **Supabase Integration**: Connected to Supabase PostgreSQL (ojvygmrxphwmaelghoxb.supabase.co) with `agent_logs` table. Backend uses service_role key, frontend uses anon key + Realtime.
2. **Agent Execution Engine** (`/app/backend/routes/agent.py`): `POST /api/run-agent` writes to Supabase, fires BackgroundTask worker, returns `{success, logId}`. `GET /api/agent-logs/{logId}` reads from Supabase.
3. **Agent Worker**: Gemini 2.5 Flash via Emergent LLM key. Writes timestamped terminal_history entries to Supabase during execution.
4. **useAgentTerminal Hook** (`/app/frontend/src/hooks/useAgentTerminal.js`): Direct port of nidoai's hook — Supabase Realtime subscription on `postgres_changes` UPDATE events filtered by `log_id`, with 1.5s fallback polling.
