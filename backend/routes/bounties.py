...
class CreateBountyRequest(BaseModel):
    ...
    reward_amount: int = Field(ge=MIN_REWARD, le=MAX_REWARD)
...
async def create_bounty(req: CreateBountyRequest, user=Depends(get_current_user())):
    """Post a new bounty. Debits the reward from the poster's wallet into escrow."""
    ...
    # Debit the escrow BEFORE inserting so a failed debit can't orphan a bounty.
    ...
    doc = {
        ...
        "reward_type": "credits",  # cash deferred
...
async def cancel_bounty(bounty_id: str, user=Depends(get_current_user())):
    ...
    return {"success": True, "refunded": doc["reward_amount"],
            "credits_remaining": refund.get("balance")}
...
async def award_bounty(bounty_id: str, req: AwardRequest,
    ...
    _emit_notification(db, sub["creator_id"], "bounty_won",
                       f"You won the bounty '{doc['title']}'! +{doc['reward_amount']} credits awarded.",
    ...
    return {
        ...
        "winner_listing_id": winner_listing_id,
        "winner_credits_balance": payout.get("balance"),
...
async def expire_lapsed_bounties(db) -> int:
    ...
    for b in candidates:
        ...
        if poster and b.get("escrow_status") == "held":
            try:
                ...
                logger.warning(f"[bounty-janitor] refund failed for {b['id']}: {e}")
                continue
        await db.bounties.update_one(
            {"id": b["id"]},
